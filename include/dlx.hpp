...
namespace wills::dlx
{
    ...
    class dancing_links
    {
        ...
        bool check_cols()noexcept
        {
            ...
            size_t right_size = 0;
            for(node<T> * ptr = root.p_right;ptr!=nullptr && ptr != &root;ptr = ptr->p_right)++right_size;

            size_t left_size = 0;
            for(node<T> * ptr = root.p_left;ptr!=nullptr && ptr != &root; ptr = ptr->p_left)++left_size;
        ...
        void init_cols(size_t col_count){
            if(headers.size()!=0)throw logic_error("dancing_links::init() on a NOT empty headers!");
            ...
            if(!check_cols())throw logic_error("column header link list error!");
        }
        ...
        /**
         * @brief get total amount of column node 
         * 
        ...
        size_t append_row(const value_t<T> values[], const size_t values_len)
        {
            ...
            for(size_t i =0;i<values_len;++i){
                if(values[i].col == 0)throw invalid_argument("append a dlx row with a zero col index.");
                if(values[i].col > headers.size())throw invalid_argument("the maximum col of append row exceed col limits.");
            }
            ...
            if(values_len == 0)return root.row_count;
        ...
        /**
         * @brief get the total amount of all the nodes in the dancing links except root and column header node
         * 
